...
function FoodOrder() {
  ...
  useEffect(() => {
    const totalTemp = orderFoods.reduce((a, b) => {
      const price = !b.discount_price
        ? b.price.value * b.quantity ?? 0
        : b.discount_price.value * b.quantity ?? 0
          ...
          <div className="d-flex">
            <div className="action-btn">
              {orderFoods && orderFoods.length > 0 && (
                <Button
                  className="action-btn__clear"
                  icon="fa-regular fa-trash-undo i-color--accent"
                  onClick={onClickOrderClear}
                />
              )}
            </div>
            <div className="action-btn ms-1">
              <Button
                className="action-btn__order br-color--accent"
                icon="fa-solid fa-cart-plus i-color--white"
              />
            </div>
          </div>
        </div>
