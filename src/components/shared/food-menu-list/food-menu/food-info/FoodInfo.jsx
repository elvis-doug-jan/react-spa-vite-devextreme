  ...
  const AddButton = () => {
    return (
      <>
        <Button
          className="btn-add"
          icon="fa-sharp fa-solid fa-plus i-color--white"
          onClick={onClickedAddFood}
        />
      </>
    )
  }
      ...
      return (
        <>
          <div className="price mt-1">
            {(+food.discount_price.value).toMoneyString()}
            <span className="price__unit">đ</span>
          </div>
          <div className="cost">
            {(+food.price.value).toMoneyString()}
            <span className="cost__unit">đ</span>
          </div>
        </>
      )
    }
    ...
    <>
      <div className="food-info row">
        <div className="col-md-2">
          <img
            className="food-info__img"
            alt={`'${food.name}'`}
            src={food.photos[0].value}
          />
        </div>
        <div className="col-md-6">
          <div className="food-info__title text-break">
            {truncate(food.name, 30)}{' '}
          </div>
          <div className="food-info__desc text-break">
